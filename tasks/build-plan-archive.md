...
### Windows Auto-Update
Add Windows platform to the auto-updater so trades on the Windows machine get updates automatically.

**What's required:**
- In the release workflow, after `npm run tauri build` on the Windows runner, zip the NSIS installer: `Trade Journal_{VERSION}_x64-setup.exe` → `Trade.Journal_{VERSION}_x64-setup.nsis.zip`
- Sign the zip using `npx tauri signer sign` with the same `TAURI_SIGNING_PRIVATE_KEY` + `TAURI_SIGNING_PRIVATE_KEY_PASSWORD` secrets (already in GitHub)
- Upload the `.nsis.zip` to the release
- Generate a combined `latest.json` that includes both `darwin-aarch64` AND `windows-x86_64` entries

**The tricky part — cross-job coordination:**
macOS and Windows build in parallel. `latest.json` needs both signatures. Current approach: macOS generates `latest.json` alone. Fix requires a third `finalize` job that runs `needs: [build-macos, build-windows]`, reads both `.sig` files from the release assets, and generates the combined `latest.json`.

**Acceptance:** Opening the app on Windows shows an update dialog when a new version is available. Windows runner must sign correctly (verify locally first before tagging).
